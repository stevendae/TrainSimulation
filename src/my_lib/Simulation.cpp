...
int getOverlapLength(const std::vector<std::shared_ptr<Node>> &path1,
                     const std::vector<std::shared_ptr<Node>> &path2)
{
    ...
    for (const auto &nodePtr : path1)
    {
        ...
        if (std::find_if(path2.begin(), path2.end(), [&nodePtr](const auto &p) {
                return p->id == nodePtr->id;
            }) != path2.end())
        {
...
bool isOpposite(const std::vector<std::shared_ptr<Node>> &path1,
                const std::vector<std::shared_ptr<Node>> &path2)
{
...
void Simulation::run(std::shared_ptr<Track> track,
                     std::vector<std::shared_ptr<Train>> &trains)
{
    std::unordered_map<int, std::vector<std::shared_ptr<Node>>> paths;
    for (const auto &train : trains)
    {
        std::cout << "Train " << train->getId() << " is travelling from "
                  << train->getStartingTrack() << " to "
                  << train->getDestination() << " in direction pointing to "
                  << train->getDirection() << std::endl;
        std::vector<std::shared_ptr<Node>> path =
            findTrainShortestPath(train, track);
    ...
    std::sort(trains.begin(),
              trains.end(),
              [&paths](const auto &a, const auto &b) {
        ...
        threads.push_back(std::thread([train, &paths, &track]() {
            ...
            for (const auto &node : path)
            {
                std::cout << "Train " << train->getId()
                          << " is travelling through node " << node->id
                          << std::endl;
...
std::vector<std::shared_ptr<Node>> Simulation::findTrainShortestPath(
    ...
    std::shared_ptr<Track> track)
{
    ...
    return track->findShortestPath(train->getStartingTrack(),
