...
int main() {
    ...
    std::cout << "The nodes on the track and their connected nodes are: " << std::endl;
    track->printTrack();
    std::cout << std::endl;
    ...
    while(inputHandler.addMoreTrains())
    {
        ...
        std::shared_ptr<Train> train = std::make_shared<Train>(++trainID, inputHandler.trainStart, inputHandler.trainDestination, inputHandler.trainDirection);
